          ...
          <center>
           <table id="sample-table-1" class="table table-striped table-bordered table-hover"> 
            ...
            <tr>
              ...
              <td>
                  <select name="" id="pid" onchange="select()">
                      <option value="0">请选择学院</option>
                      @foreach($data as $v)
                      <option class="c_id" value="{{ $v['c_id'] }}">{{ $v['c_name'] }}</option>
                      @endforeach
                  </select>
              </td>
            </tr>
...
<script src="jquery.js"></script>
 <script type="text/javascript">
    function select(){
      var c_id = $("#pid").val();
      // alert(c_id);
      $.get('select_class',{'c_id':c_id},function(e){
          var str = '';
          var e = eval(e);
          // alert(e);
          for(var i in e){
            str+='<option>'+e[i]['c_name']+'</option>';
          }
          $("#classname").html(str);
      });
    }
    $("#sub").click(function(){
      ...
      var pid = $("#pid").val();
        if(c_name == ''){
          ...
          $.get('class_add',{'c_name':c_name,'pid':pid},function(msg){
            if (msg==1) {
              alert('班级添加完成');
            }else{
              alert('该班级已存在，请重新添加。');
